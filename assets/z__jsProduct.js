...
var dataAttribute = document.querySelector('[data-messaging]');
priceUITemplate.appendChild(document.createElement('div')).textContent= dataAttribute.getAttribute('data-messaging');
  ...
  enableSlideshow(selector, settings) {
      ...
      $slides.each((index, slide) => {
        ...
        if ($slide.hasClass('is-selected')) {
          if (mediaType === 'video') {
            ...
            $slide.find('.plyr__control--overlaid').click();
    ...
    function checkForVideosPause() {
      $slides.each((index, slide) => {
        ...
        if ($slide.find('.plyr__control--overlaid').hasClass('plyr__control--pressed')) {
           $slide.find('.plyr__control--overlaid').click();
        }
        }
      });
    ...
    if (thumbnailsEnabled === true && thumbnailsSliderEnabled === true && $slides.length > 1) {
      ...
      if (window.isScreenSizeLarge()) {
        if (thumbnailsPosition === 'right-thumbnails' || thumbnailsPosition === 'left-thumbnails') {
         ...
         //    prevNextButtons: $thumbnails.length > 5 ? arrowsEnabled : true,
