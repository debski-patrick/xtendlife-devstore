...
window.PXUTheme.jsHeader = {
  init: function($section) {
    ...
    // Overlaid header
    if (this.enable_overlay === true && isScreenSizeLarger()) {
      this.updateOverlayStyle(this.sectionUnderlayIsImage());
    }

    // Sticky header
    if (this.enable_sticky === true && isScreenSizeLarger()) {
    ...
    if (isScreenSizeLarger() || !/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
  ...
  toggleMenuClick: function(e, $this) {
    ...
    if ($parentWrap.find('.mega-menu__section').css('display') == 'block' && $parentWrap.find('.mega-menu__section').hasClass('is-active')) {
      $parentWrap.find('.mega-menu__section').hide();
    }
     else{
       $parentWrap.find('.mega-menu__section').show();
     }
